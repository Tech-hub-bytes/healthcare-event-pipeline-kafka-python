...
from pipeline.fhir_parser import parse_fhir_resource
...
def main() -> None:
    ...
    file_path = Path(file_arg).resolve() if file_arg else PATHS["samples"] / "emma-fhir-bundle.json"
    raw = file_path.read_text(encoding="utf-8")
    file_name = file_path.name

    patient_hint = "Unknown patient"
    try:
        preview = parse_fhir_resource(raw, file_name)
        patient_hint = preview["patient"]
    except Exception:  # noqa: BLE001
        pass

    envelope = build_envelope(
        event_type="fhir.resource.received",
        source_system="local-fhir-producer",
        patient_name=patient_hint,
        content_type="application/fhir+json",
        payload=raw,
        extras={"source_file": file_name, "source_path": str(file_path), "feed": "fhir"},
    )
    ...
    producer = Producer({"bootstrap.servers": KAFKA_BROKERS, "client.id": "healthcare-fhir-producer"})
    producer.produce(
        TOPICS["fhir_raw"],
